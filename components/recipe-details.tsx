...
import { formatDate, getReasonablePrice, getTotalCostFromIngredients } from "@/lib/utils"
...
export function RecipeDetails({ recipe, settings, onClose, onEdit }: RecipeDetailsProps) {
  return (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-2">
            ...
            {typeof recipe.price === 'number' && recipe.price > 0 && (
              (() => {
                ...
                const linemanGPNum = settings?.linemanGP ?? 0;
                const linemanMultiplier = 1 - (linemanGPNum / 100);
                const grabGPNum = settings?.grabGP ?? 0;
                const grabMultiplier = 1 - (grabGPNum / 100);

                const linemanPrice = typeof recipe.linemanPrice === 'number' && recipe.linemanPrice > 0 ? recipe.linemanPrice : (recipe.price / linemanMultiplier);
                const linemanProfit = (linemanPrice * linemanMultiplier) - totalWithOther;

                const grabPrice = typeof recipe.grabPrice === 'number' && recipe.grabPrice > 0 ? recipe.grabPrice : (recipe.price / grabMultiplier);
                const grabProfit = (grabPrice * grabMultiplier) - totalWithOther;
